...
class HostedAuthoringAppContract {
  ...
  factory HostedAuthoringAppContract.fromJson(Map<String, Object?> json) {
    _requireExactFields(
      json,
    ...
    return HostedAuthoringAppContract(
...
class HostedAuthoringContractApi {
  HostedAuthoringContractApi({required Uri baseUri, http.Client? client})
  ...
  Future<List<HostedAuthoringAppContract>> listApps({
    ...
    final List<HostedAuthoringAppContract> apps =
        <HostedAuthoringAppContract>[];
    ...
    for (final Object? rawApp in rawApps) {
      ...
      final HostedAuthoringAppContract app =
          HostedAuthoringAppContract.fromJson(rawApp);
    ...
    return List<HostedAuthoringAppContract>.unmodifiable(apps);
  }
...
List<String> _requiredFormats(Map<String, Object?> json, String key) {
  ...
  for (final Object? value in raw) {
    if (value is! String || !_contentFormatPattern.hasMatch(value)) {
      throw FormatException(
        'JSON field $key contains an invalid content format.',
      );
    }
    if (!seen.add(value)) {
      throw FormatException(
        'JSON field $key contains a duplicate content format.',
      );
    }
    formats.add(value);
  }
