...
(function installGirlsAppSourceEditor() {
  ...
  async function fetchSource(app) {
    ...
    if (!Number.isInteger(revision) || revision < 1) throw new Error("編集版を確認できませんでした。");
  ...
  saveButton.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!publish.ok) {
        ...
        throw new Error(`編集版は保存されましたが、公開版の更新に失敗しました: ${await responseError(publish)}`);
      }
