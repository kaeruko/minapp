...
みんアプは、最初から自分でコードを書けることを前提にしない。まずはChatGPTなどの生成AIに「こんなアプリを作りたい」と相談しながら形にして、できたWebアプリを教室へアップロードする。そこからもっとやってみたくなった子は、HTML/CSS/JavaScriptを読んだり、自分で直したりしながら少しずつ学んでいける。

子どもごとにGoogle Play Consoleやクラウドの契約を持たなくても、ChatGPTと一緒に作ったものも、自分でコードを書いたものも、先生の承認後にグループ内で使えるようにする。

## コンセプト

みんアプの入口は「プログラミングを勉強してから作る」ではなく、**まず作って、使ってみる**こと。

1. 作りたいものを言葉で考える
2. ChatGPTなどの生成AIと一緒にWebアプリを作る
3. みんアプへアップロードする
4. 先生が安全性を確認する
5. クラスのみんなで使う
6. 興味が出たら、コードを読んだり直したりして次へ進む

生成AIを使うことを「手抜き」ではなく、アイデアを形にするための標準的な入口として扱う。一方で、生成AIの利用を必須にはせず、自分で作ったアプリも同じように共有できる。
