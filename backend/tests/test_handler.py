...
class FakeBackend:
    ...
    def create_student(
        ...
        resolved_login_id = login_id or "student-12345678"
        return {"user_id": "c" * 32, "login_id": resolved_login_id, "temporary_password": "ExamplePassword1", "group_id": group_id, "role": "student"}
...
class HandlerTests(unittest.TestCase):
    ...
    def test_create_student_accepts_requested_login_id(self) -> None:
        group_id = "a" * 32
        response = handler.lambda_handler(_event("POST", f"/groups/{group_id}/students", body={"login_id": "yamada"}, subject="teacher-sub"), None)
        self.assertEqual(response["statusCode"], 201)
        payload = json.loads(response["body"])
        self.assertEqual(payload["login_id"], "yamada")
        self.assertEqual(self.backend.last_call, ("create_student", "teacher-sub", group_id, "yamada"))

    def test_create_student_keeps_automatic_id_compatibility(self) -> None:
        group_id = "a" * 32
        response = handler.lambda_handler(_event("POST", f"/groups/{group_id}/students", body={}, subject="teacher-sub"), None)
        self.assertEqual(response["statusCode"], 201)
        self.assertEqual(self.backend.last_call, ("create_student", "teacher-sub", group_id, None))

    def test_create_student_rejects_invalid_requested_login_id(self) -> None:
        group_id = "a" * 32
        response = handler.lambda_handler(_event("POST", f"/groups/{group_id}/students", body={"login_id": "Yamada"}, subject="teacher-sub"), None)
        self.assertEqual(response["statusCode"], 400)
        self.assertIsNone(self.backend.last_call)
