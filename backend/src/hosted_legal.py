...
1. サービスについて
みんアプ Hosted は、小さなWebアプリや創作物を作成・保存・実行・共有するためのサービスです。グループ機能を利用する場合は、招待された利用者どうしで非公開グループ内のアプリや創作物を共有できます。グループや招待コードは、意図した相手だけに共有してください。
...
5. アカウントとグループ管理
アカウントの認証情報やリカバリーコードを第三者へ不用意に開示しないでください。グループ機能を利用する場合は、招待コードも不用意に開示せず、グループのオーナーはメンバー管理と共有範囲について責任を持ちます。
...
1. 取得・処理する情報
みんアプ Hosted では、サービス提供に必要な範囲で、ログインID、内部ユーザーID、認証システム上の識別子、リカバリーコードのハッシュ、利用者が任意で紐づけたメールアドレスと確認状態、アプリのメタデータ、Runtime state、利用者がアップロード・公開したコンテンツ、規約同意versionと同意時刻を処理します。グループ機能を利用する場合は、グループ所属情報も処理します。
...
3. 利用目的
取得・処理した情報は、本人認証、アカウント復旧、利用者が指定したメールアドレスの確認とアカウントへの紐づけ、アプリ実行と状態保存、コンテンツ提供、不正利用・迷惑行為の防止、セキュリティ対応、障害調査、サービス運用のために利用します。グループ機能を利用する場合は、グループ内での共有とメンバー管理にも利用します。
